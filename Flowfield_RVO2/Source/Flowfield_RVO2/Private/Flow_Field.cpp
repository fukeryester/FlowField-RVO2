...
void AFlow_Field::BeginPlay()
{
    ...
    {
        GetWorldTimerManager().SetTimer(UpdateTimerHandle, this, &AFlow_Field::UpdateFlowField, UpdateInterval, true);
    }
    ...
    {
        DrawDebugFlowField();
        GetWorldTimerManager().SetTimer(DebugDrawTimerHandle, this, &AFlow_Field::DrawDebugFlowField, 2.0f, true);
    }
}

void AFlow_Field::Tick(float DeltaTime)
{
...
void AFlow_Field::InitializeFlowField()
{
    // 计算流场原点（以Actor位置为中心）
...
void AFlow_Field::UpdateFlowField()
{
    if (!TargetActor)
    {
...
void AFlow_Field::DetectObstacles()
{
    // 遍历所有网格，检测障碍物
...
void AFlow_Field::CalculateHeatMap()
{
    if (!TargetActor)
        return;
...
void AFlow_Field::CalculateVectorField()
{
    // 遍历所有网格，计算流向向量
    for (int32 Y = 0; Y < GridHeight; ++Y)
    {
...
FVector2D AFlow_Field::GetFlowVectorAtLocation(const FVector& WorldLocation) const
{
...
bool AFlow_Field::IsLocationValid(const FVector& WorldLocation) const
{
    FVector2D GridLocation = WorldToGrid(WorldLocation);
    return IsGridIndexValid(GridLocation);
}

FVector2D AFlow_Field::WorldToGrid(const FVector& WorldLocation) const
{
...
FVector AFlow_Field::GridToWorld(const FVector2D& GridLocation) const
{
...
int32 AFlow_Field::GridToIndex(const FVector2D& GridLocation) const
{
    return GridLocation.Y * GridWidth + GridLocation.X;
}

FVector2D AFlow_Field::IndexToGrid(int32 Index) const
{
    return FVector2D(Index % GridWidth, Index / GridWidth);
}

bool AFlow_Field::IsGridIndexValid(const FVector2D& GridLocation) const
{
    return GridLocation.X >= 0 && GridLocation.X < GridWidth &&
           GridLocation.Y >= 0 && GridLocation.Y < GridHeight;
}

TArray<FVector2D> AFlow_Field::GetNeighborDirections() const
{
    return {
        FVector2D(0, 1),   // 上
        FVector2D(1, 0),   // 右
...
TArray<FVector2D> AFlow_Field::GetEightNeighbors(const FVector2D& GridLocation) const
{
...
void AFlow_Field::DrawDebugFlowField()
{
    if (!GetWorld()) return;
...
FVector2D AFlow_Field::CalculateObstacleRepulsionVector(const FVector2D& GridLocation) const
{
    if (!bEnableObstacleRepulsion)
    {
        return FVector2D::ZeroVector;
...
void AFlow_Field::DrawObstacleRepulsionVectors() const
{
...
FVector2D AFlow_Field::FindNearestNonObstacleLocation(const FVector2D& GridLocation) const
{
    // 使用广度优先搜索寻找最近的非障碍区域
    TArray<FVector2D> Queue;
