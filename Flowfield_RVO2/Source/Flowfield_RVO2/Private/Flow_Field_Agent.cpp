...
void AFlow_Field_Agent::BeginPlay()
{
    Super::BeginPlay();
    ...
    {
        GetWorldTimerManager().SetTimer(DirectionUpdateTimerHandle, this, &AFlow_Field_Agent::OnDirectionUpdateTimer, UpdateDirectionInterval, true);
    }
...
void AFlow_Field_Agent::Tick(float DeltaTime)
{
...
void AFlow_Field_Agent::SetFlowField(AFlow_Field* NewFlowField)
{
...
void AFlow_Field_Agent::UpdateMovementDirection()
{
...
FVector2D AFlow_Field_Agent::GetBilinearInterpolatedFlowVector(const FVector& WorldLocation) const
{
    if (!FlowField)
...
TArray<FVector2D> AFlow_Field_Agent::GetOccupiedGridCells(const FVector& WorldLocation) const
{
...
void AFlow_Field_Agent::OnDirectionUpdateTimer()
{
    UpdateMovementDirection();
}

void AFlow_Field_Agent::UpdateMeshScale()
{
    if (MeshComponent)
    {
        // 根据AgentRadius调整网格大小
...
FVector2D AFlow_Field_Agent::BilinearInterpolate(const TArray<FVector2D>& CornerVectors, const FVector2D& Weights) const
{
...
// RVO2避障实现
FVector2D AFlow_Field_Agent::CalculateRVO2Velocity(const FVector2D& PreferredVelocity)
{
    return ComputeNewVelocity(PreferredVelocity);
}

TArray<AFlow_Field_Agent*> AFlow_Field_Agent::GetNearbyAgents() const
{
    TArray<AFlow_Field_Agent*> NearbyAgents;
    ...
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), AFlow_Field_Agent::StaticClass(), FoundAgents);
    
    FVector CurrentLocation = GetActorLocation();
    
    for (AActor* Actor : FoundAgents)
    {
        AFlow_Field_Agent* Agent = Cast<AFlow_Field_Agent>(Actor);
        if (Agent && Agent != this)
        {
...
FVelocityObstacle AFlow_Field_Agent::ComputeVelocityObstacle(const AFlow_Field_Agent* OtherAgent) const
{
    if (!OtherAgent)
...
bool AFlow_Field_Agent::IsVelocityInObstacle(const FVector2D& Velocity, const FVelocityObstacle& Obstacle) const
{
    // 检查速度是否在速度障碍物内
...
FVector2D AFlow_Field_Agent::FindBestVelocity(const FVector2D& PreferredVelocity, const TArray<FVelocityObstacle>& Obstacles) const
{
    // 如果没有障碍物，直接返回首选速度
    if (Obstacles.Num() == 0)
    {
...
FVector2D AFlow_Field_Agent::ComputeNewVelocity(const FVector2D& PreferredVelocity)
{
    // 获取附近的Agent
    TArray<AFlow_Field_Agent*> NearbyAgents = GetNearbyAgents();
    
    // 计算所有速度障碍物
    TArray<FVelocityObstacle> Obstacles;
    for (AFlow_Field_Agent* Agent : NearbyAgents)
    {
...
void AFlow_Field_Agent::DrawRVO2DebugInfo()
{
    if (!GetWorld())
        return;
